...
public class MatrixBuilderOnTheFlyWithStateVectors {
    ...
    /**
     * Führt die Zustandsraumexploration durch.
     * @param doWrite Wenn true, wird die Matrix in eine Datei geschrieben. Wenn false (für Warm-up), wird nur gerechnet.
     * @param outputFile Pfad zur Ausgabedatei (nur relevant, wenn doWrite true ist).
     * @return Anzahl der gefundenen Zustände.
     */
    public int build(boolean doWrite, String outputFile) {
        // 1. Consumer-Thread (Writer) nur initialisieren und starten, wenn doWrite true ist
        Thread writerThread = null;
        if (doWrite) {
            writerThread = new Thread(() -> {
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFile))) {
                    while (true) {
                        // Blockiert, bis ein Record da ist
                        TransitionRecord record = queue.take();

                        // Check für Giftpille zum Beenden
                        if (record == POISON_PILL) {
                            break;
                        }
                        writer.write(record.sourceID() + " " + record.targetID() + " " + record.rate() + "\n");
                    }
                    System.out.println("\n[Consumer] Datei erfolgreich geschrieben und geschlossen.");
        ...
        System.out.println(doWrite ? "Starting full Exploration (with I/O)..." : "Starting Warm-up (Memory only)...");
        long startTime = System.nanoTime();
        long iter = 0L;

        // 3. Hauptschleife der Exploration
        while (!worklist.isEmpty()) {
            int BIAS = computeBias(worklist.size());
            final Object[] currentState = ((++iter % BIAS) == 0)
                    ? worklist.pollFirst()   // Breitensuche-Anteil
                    : worklist.pollLast();    // Tiefensuche-Anteil

            if (currentState == null) continue;

            final int sourceId = stateToId.getInt(currentState);

            // Fortschrittsanzeige alle 100.000 Zustände
            if (visitedStates.size() % 100000 == 0) {
                printProgress(startTime);
            }

            // Spezialfall: Globaler Endzustand (Self-Loop für PRISM/DTMC falls nötig)
            ...
            // Sink-Definition: Hier werden neu gefundene Transitionen verarbeitet
            final TransitionSink sink = (Object[] target, String rate) -> {
                ...
                // NUR in die Queue schreiben, wenn doWrite aktiv ist
                if (doWrite) {
                    try {
            ...
            // Interleaving-Schritt für alle Prozesse
            for (String processName : this.processNames) {
                final Integer pcIdx = this.processNameToPcIndex.get(processName);
                if (pcIdx == null) continue;

                final int pcId = (Integer) currentState[pcIdx];
        ...
        // 4. Abschluss: Writer-Thread beenden (Giftpille)
        if (doWrite && writerThread != null) {
            try {
                queue.put(POISON_PILL);
                writerThread.join();
            } catch (InterruptedException e) {
                System.err.println("Fehler beim Warten auf den Writer-Thread.");
            }
        }

        System.out.println("\nDone. States: " + visitedStates.size() + " | Transitions: " + transitionCount.get());
        return this.visitedStates.size();
    }
